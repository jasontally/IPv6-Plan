...
describe('IPv6 Formatting (RFC 5952)', () => {
  ...
  it('should use lowercase hex digits', () => {
    const bytes = new Uint8Array([0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
    expect(bytes.length).toBe(16);
  });

  it('should omit leading zeros in each group', () => {
    const bytes = new Uint8Array([0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
...
describe('Prefix Masking', () => {
  ...
  it('should apply /48 prefix correctly', () => {
    const bytes = new Uint8Array([0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0xff, 0xff,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
